...
TcpConnection::TcpConnection(Socket socket, EventLoop* loop, ThreadPool* threadPool)
...
// 发送数据的方法
void TcpConnection::send(const MessageHeader& header, const std::vector<char>& body) {
...
void TcpConnection::sendMessage(uint16_t type, const std::string& data) {
    if (closed_) {
...
void TcpConnection::handleMessage(const MessageHeader& header, const std::vector<char>& body) {
    if (messageCallback_) {
